...
class FirebaseHelper {
  ...
  Future<String> insertRequest(RequestModel requestModel) async {
    CollectionReference reference =
        FirebaseFirestore.instance.collection(requestCollection);

    await reference.add(requestModel.toMap());
    return requestModel.bookingDate.toString();
  }

  Future updateQuoteStatus(String id, String status,
      {String paymentStatus = PaymentType.cod}) async {
    CollectionReference reference =
  ...
  Future insertPayout(
      ...
      int time}) async {
    CollectionReference reference =
        FirebaseFirestore.instance.collection(payoutCollection);
    PendingPayoutModel model = PendingPayoutModel(
        agent: agent,
        amount: amount,
        bookingId: bookingId,
        time: time,
  ...
  Future<CouponModel> validateCoupon(String coupon) async {
    CollectionReference reference =
        FirebaseFirestore.instance.collection(FirebaseHelper.couponCollection);
  ...
  Future<bool> checkCouponUsage(String coupon) async {
    CollectionReference reference = FirebaseFirestore.instance
        .collection(FirebaseHelper.couponUsageCollection);
    final d = await reference
  ...
  Future<void> insertCouponUsage(
      {QuoteModel quoteModel, String coupon, double discountPrice}) async {
    CollectionReference reference = FirebaseFirestore.instance
        .collection(FirebaseHelper.couponUsageCollection);
    await reference.add({
      'code': coupon,
